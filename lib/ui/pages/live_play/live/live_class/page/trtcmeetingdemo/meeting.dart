...
import '../../page/trtcmeetingdemo/index.dart';
import 'tool.dart';
...
class MeetingPageState extends State<MeetingPage> with WidgetsBindingObserver {
  ...
  enterRoom() async {
    try {
      userInfo['userSig'] = await GenerateTestUserSig.genTestSig(userInfo['userId']);
  ...
  onRtcListener(type, param) async {
    if (type == TRTCCloudListener.onError) {
      if (param['errCode'] == -1308) {
        MeetingTool.toast('屏幕记录失败', context);
    ...
    if (type == TRTCCloudListener.onEnterRoom) {
      if (param > 0) {
        MeetingTool.toast('进入房间成功', context);
      }
    }
    ...
    if (type == TRTCCloudListener.onUserVideoAvailable) {
      ...
      if (param['available']) {
        for (var i = 0; i < userList.length; i++) {
          if (userList[i]['userId'] == userId && userList[i]['type'] == 'video') {
    ...
    if (type == TRTCCloudListener.onUserSubStreamAvailable) {
      ...
      if (param["available"]) {
        ...
        for (var i = 0; i < userList.length; i++) {
          if (userList[i]['userId'] == userId && userList[i]['type'] == 'subStream') {
            if (isDoubleTap &&
                doubleUserId == userList[i]['userId'] &&
                doubleUserIdType == userList[i]['type']) {
              doubleTap(userList[i]);
  ...
  doubleTap(item) async {
    ...
    if (item['userId'] == userInfo['userId']) {
      ...
      userList.add(item);
      if (item['type'] == 'video') {
        await trtcCloud.stopRemoteView(item['userId'], TRTCCloudDef.TRTC_VIDEO_STREAM_TYPE_BIG);
      } else {
        await trtcCloud.stopRemoteView(item['userId'], TRTCCloudDef.TRTC_VIDEO_STREAM_TYPE_SUB);
      }
      if (isDoubleTap) {
  ...
  Widget videoVoice(item) {
    return Positioned(
      child: new Container(
        ...
        Text(
          item['userId'] == userInfo['userId'] ? item['userId'] + "(me)" : item['userId'],
        ...
        child: new Container(
          ...
          child: new Row(
            ...
            children: <Widget>[
              ...
              Text(meetId.toString(), style: TextStyle(fontSize: 20, color: Colors.white)),
              TextButton(
              ...
              IconButton(
                  ...
                  onPressed: () {
                    if (isOpenCamera) {
                      ...
                      if (isDoubleTap && doubleUserId == userList[0]['userId']) {
                        ...
                        (index) => LayoutBuilder(
                          ...
                          builder: (BuildContext context, BoxConstraints constraints) {
                            Size size = MeetingTool.getViewSize(
                                MediaQuery.of(context).size, userList.length, index, item.length);
                            ...
                            return Container(
                              ...
                              child: Stack(
                                key: valueKey,
                                children: <Widget>[
                                  renderView(item[index], valueKey, width, height),
