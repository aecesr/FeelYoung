...
import '../../page/trtcmeetingdemo/tool.dart';
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Expanded(
                                    flex: 1,
                                    child: Offstage(
                                      offstage: item['userId'] == userInfo['userId'],
                                      child: IconButton(
                                          icon: Icon(
                                          ...
                                          onPressed: () {
                                            if (micMap[item['userId']] == null) {
                                              micMap[item['userId']] = true;
                                              trtcCloud.muteRemoteAudio(item['userId'], true);
                                            } else {
                                              micMap.remove(item['userId']);
                                              trtcCloud.muteRemoteAudio(item['userId'], false);
                                            }
                                            this.setState(() {});
                                          }),
                                    ),
                    ...
                    children: <Widget>[
                      ElevatedButton(
                        style: ElevatedButton.styleFrom(
                            textStyle: const TextStyle(color: Color.fromRGBO(245, 108, 108, 1))),
                        onPressed: () {
                          trtcCloud.muteAllRemoteAudio(true);
                          MeetingTool.toast('全体静音', context);
                          for (var i = 0; i < micList.length; i++) {
                            micMap[micList[i]['userId']] = true;
                          }
                          this.setState(() {});
                        },
                        child: Text('全体静音', style: TextStyle(color: Colors.white)),
                      ),
                      ElevatedButton(
                        style: ElevatedButton.styleFrom(
                            textStyle: const TextStyle(color: Color.fromRGBO(64, 158, 255, 1))),
                        onPressed: () {
                          trtcCloud.muteAllRemoteAudio(false);
                          MeetingTool.toast('解除静音', context);
                          this.setState(() {
                            micMap = {};
                          });
                        },
                        child: Text('解除静音', style: TextStyle(color: Colors.white)),
                      ),
                    ],
                  ),
