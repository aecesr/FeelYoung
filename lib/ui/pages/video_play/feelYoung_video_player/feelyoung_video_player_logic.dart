...
import 'feelYoung_video_player_state.dart';

class FeelYoungVideoPlayerLogic extends GetxController {
  final FeelYoungVideoPlayerState state = FeelYoungVideoPlayerState();
  ...
  void fetchDanMu(beginProgress) {
    ...
    HYDanMuRequest.getDanMuProtoData(state.oid, 1).then((value) {
      ...
      for (var i = 0; i < state.danMuRouteAmount; i++) {
        Widget footer = Container(width: 1.sw);
        state.danMuChildren[i].add(footer);
        Widget widget = GetBuilder<FeelYoungVideoPlayerLogic>(builder: (logic) {
          return SizedBox(
            width: 1.sw,
            height: 15.w,
