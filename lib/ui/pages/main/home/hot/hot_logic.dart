...
class HotLogic extends GetxController {
  ...
  // }
  ///初始化界面
  void initHomeHotWidgets() {
    ///还没有数据就初始化数据
    if (state.homeHotWidgets.isEmpty) {
      if (state.feedIndexItemList.length == 10) {
        ///单纯的十条数据
        state.homeHotWidgets
            .add(buildHomeHotVideoCards(state.feedIndexItemList));
      } else if (state.feedIndexItemList.length == 11) {
        ///有轮播图的是十一条数据
        state.homeHotWidgets.addAll([
          ///前三条视频数据为轮播图数据
          buildHomeHotCarousel(state.feedIndexItemList[0].bannerItems!),

          ///后面的数据用于card
          buildHomeHotVideoCards(state.feedIndexItemList.sublist(1)),
        ]);
      } else {
        if (Constant.isDebug) {
  ...
  ///视频列表
  Widget buildHomeHotVideoCards(List<FeedIndexItem> items) {
    return GridView.builder(
      ...
      itemBuilder: (ctx, index) {
        ...
        return buildHotItem(items[index]);
        // return Container();
      },
    );
  }
  ...
  ///视频块
  Widget buildHotItem(FeedIndexItem video) {
  ...
  void loadHotItemData() {
    HYHomeRequest.getFeedIndexData(fetchFeedIndexParamsWithSign())
      ...
      state.homeHotWidgets
          .add(buildHomeHotVideoCards(value.data.items.sublist(0, 8)));
      update();
    });
  }
  ...
  ///轮播图
  Widget buildHomeHotCarousel(List<BannerItem> bannerItems) {
    return ClipRRect(
      borderRadius: BorderRadius.circular(8.r),
      child: Container(
